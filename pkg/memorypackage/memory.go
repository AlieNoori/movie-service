...
func (r *Registry) ServiceAddresses(ctx context.Context, serviceName string) ([]string, error) {
	...
	var res []string
	for instanceID, instanceService := range r.serviceAddrs[serviceName] {
		if instanceService.lastActive.Before(time.Now().Add(-5 * time.Second)) {
			log.Println("Instance " + instanceID + " of service " + serviceName + " is not active, skipping")
			continue
		}
		res = append(res, instanceService.hostPort)
	}
