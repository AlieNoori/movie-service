...
import (
	...
	memory "movieexample.com/pkg/discovery/memorypackage"
...
func main() {
	...
	if _, err := metadataClient.PutMetadata(ctx, &gen.PutMetadataRequest{Metadata: metadata}); err != nil {
		log.Fatalf("put metadata: %s", err)
	}

	log.Println("Retrieving test metadata via metadata service")
	getMetadataResp, err := metadataClient.GetMetadata(ctx, &gen.GetMetadataRequest{MovieId: metadata.Id})
	if err != nil {
		log.Fatalf("get metadata: %v", err)
	}

	if diff := cmp.Diff(getMetadataResp.Metadata, metadata, cmpopts.IgnoreUnexported(gen.Metadata{})); diff != "" {
	...
	getMovieDetailsResp, err := movieClient.GetMovieDetails(ctx, &gen.GetMovieDetailsRequest{MovieId: metadata.Id})
	if err != nil {
	...
	if got, want := getAggregatedRatingResp.RatingValue, float64(firstRating); got != want {
